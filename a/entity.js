...
export class Entity {
    ...
    update(deltaTime) {
        ...
        if (this.currentTask === 'storing traded goods' && this.targetNode && this.isAtTarget()) {
    ...
    hasTradedGoods() {
        return this.tradeInventory.items.length > 0;
    }
    ...
    gatherFromTargetNode() {
        if (!this.targetNode || this.isInventoryFull()) {
            ...
            if (this.isInventoryFull()) {
                ActionHandler.depositResources(this);
            } else {
                this.currentTask = 'idle';
            }
            return;
    ...
    giveResource(type, amount) {
        ...
        const depositPoint = this.getDepositPoint();
        if (depositPoint && depositPoint.inventory[type]) {
            depositPoint.inventory[type] = Math.max(0, depositPoint.inventory[type] - amount);
            depositPoint.inventory[type] = Math.round(depositPoint.inventory[type] * 10) / 10;
        } else {
            // Use personal resources if no storage
